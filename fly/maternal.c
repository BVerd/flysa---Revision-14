...
List2Interp( Dlist * inlist, Input * inp, int ngenes ) {
    ...
    if( !inp->zyg.full_lin_start ) {
        if( !( inp->zyg.full_lin_start = ( int * ) 
            calloc( inp->zyg.defs.ndivs+1, sizeof( int ) ) ) ) {
    ...
    for( current = inlist; current; current = current->next ) {
        ...
        for( j = 1; j <= ngenes; j++ ) {        /* always: read concs into array */
            ...
            D->record[ D->size-1 ].array[ 
                D->record[ D->size-1 ].size-1 ].conc = current->d[j];
            D->record[ D->size-1 ].array[ 
                D->record[ D->size-1 ].size-1 ].index = i;
            i++;

        }
        /* initialize lin_start: this array is later used by Blastoderm and such   */
        if( ParseLineage( 
                inp->zyg.full_lin_start[ inp->zyg.defs.full_ccycles-1 ] ) != 
                ParseLineage( current->lineage ) ) {
            inp->zyg.defs.full_ccycles++;
            if( !( inp->zyg.full_lin_start = ( int* ) realloc( 
                    inp->zyg.full_lin_start, 
                    inp->zyg.defs.full_ccycles * sizeof( int ) ) ) )
                error( "List2Interp: could not allocate full_lin_start array" );
            inp->zyg.full_lin_start[ inp->zyg.defs.full_ccycles-1 ] = 
                current->lineage;
        }
