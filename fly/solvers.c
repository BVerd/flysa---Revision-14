...
/*
 * delay solver related static variables...
 */
// maximum and minimum tau
double maxdel, mindel;
// number of delay equations (= nr of gap genes); used by DCERk32, y_delayed
int numdel;   
// for the heuristic?
int gridstart;
// static array of the gap genes' tau; set in SoDe, used by DCERk32
double *delay;
 ...
 *
 * *vin     - input values in a 1d array
 * *vout    - output of the delay solver
 * tin      - time point of the input values
 * tout     - time point at which we want output
 * stephint - starting step size (adaptive?)
 * accuracy - "max allowed error during stepping"
 * n        - array size?
 * slog     - solver log for debugging purposes I assume
 * *si      - history and external inputs to solvers
 * *inp     - the whole input, and nothing but the input
 */
void
SoDe( double *vin, double *vout, double tin, double tout, double stephint, 
    double accuracy, int n, FILE *slog, SolverInput *si, Input * inp ) {
    // some counters
    int i, j;
    // local copy of the equation parameters
    EqParms *lp;
    // 1d arrays?
    double *shift_discs, *fact_discons;
    // discontinuities array?
    double *discon_array;
    // length of discontinuity array?
    int num_discons, fact_discons_size;

    // 2d arrays?
    double **vees, *tees;
    
    //printf( "DBG running delay solver\n" );
    /*lp = GetMutParameters(); */
    lp = &( inp->lparm );
    // ?
    fact_discons = GetFactDiscons( &fact_discons_size, si->all_fact_discons );

    //printf( "%d %f %f %f\n", fact_discons_size, fact_discons[0],
    //    fact_discons[1], fact_discons[2] );

    // keep the delay -- value of tau -- within bounds
    maxdel = 0.;
    mindel = 1000.;
    ...
    // number of delay variables = nr of gap genes
    numdel = inp->zyg.defs.ngenes;
    delay = lp->tau;

    //printf( "DBG maxdel:%f, mindel:%f and numdel: %d\n", 
    //    maxdel, mindel, numdel );

    // find a j that is smaller than the total size of fact_discons, while
    // tout is smaller than the item that j refers to
    j = -1;
    do {
        j++;
    } while( ( j < fact_discons_size ) && ( tout > fact_discons[j] ) );

    // what is shift_discs? shifted discontinuities
    shift_discs = ( double * ) calloc( j + 1, sizeof( double ) );
    shift_discs[ 0 ] = 0.0;
    // filling up shift_discs with discontinuity time points minus the starting
    // time point of the solver
    for( i = 0; i < j; i++ ) {
        shift_discs[i + 1] = fact_discons[i] - tin;
    }
    // create an array with all timepoints that we should record?
    discon_array = Construct_Discont_Array( tout - tin, lp->tau, 
        inp->zyg.defs.ngenes, shift_discs, j + 1, &num_discons );
    for( i = 0; i < num_discons; i++ ) {
        discon_array[i] += tin;
    }

    // I'm not sure what memory-wizardry is going on here.. it seems unneeded
    ...
    tees[0] = tin;
    tees[1] = tout;
    // do the actual solving of the delay equations
    DCERk32( vees, n, tees, 2, discon_array, num_discons, stephint, 
        accuracy, si, inp );

    // clean up
    free( shift_discs );
    if( discon_array ) free( discon_array );
    free( vees );
    free( tees );
}
...
compare( double *x, double *y ) {
    // if x > y return 1, if x < y return -1, else 0
    if( *x > *y ) {
        return 1;
    } else if( *x < *y ) {
        return -1;
...
/*
 * range      - delta time, ie t_out minus t_in
 * *taus      - gap gene delays
 * n          - size of taus
 * *starts    - shift discontinuity array
 * sn         - size of discontinuity array
 * *disc_size - number of discontinuities (returned)
 */
double *
Construct_Discont_Array( double range, double *taus, int n, double *starts, 
    int sn, int *disc_size ) {

    // number of times the delay fits in the time range
    long m;
    ...
    // for each gap gene
    for( i = 0; i < n; i++ ) {
        // if delay is not zero, check how many delays fit in the time range 
        if( taus[ i ] != 0.0 ) {
        ...
        // cap at 5, but why? especially as m is a long!
        if( m > 5 ) m = 5;

        //printf( "DBG %f %f m=%d\n", range / taus[ i ], 
        //    floor( range / taus[ i ] ), m );

        // for each element in the discontinuity array
        for( k = 0; k < sn; k++ ) {
            // if the start time (fact_disc - tin) is bigger than -tau?
            if( starts[ k ] > -taus[ i ] ) {
                // reallocate a delay array, size M + max(5)
                delay_array = ( double * ) realloc( delay_array, ( M + m ) * sizeof( double ) );
                // and fill the new part of the array with the explicit times
                // at which we need the history
                for( j = M; j < M + m; ++j ) {
                    delay_array[ j ] = starts[ k ] + ( j - M + 1 ) * taus[ i ];
                }
                // M accumulates the nr of delays of each gap gene
                M += m;
            }
        }
    }
    
    // sort all the time values in the delay array
    /* for( i = 0; i < M; i++ ) printf( "DBG %1.14f\n", delay_array[ i ] ); */
    qsort( ( void * ) delay_array, M, sizeof( double ),
        ( int ( * )( const void *, const void * ) ) compare );
    /* for( i = 0; i < M; i++ ) printf( "DBG %1.14f\n", delay_array[ i ] ); */
    // also sort the taus, but this mixes up gap gene order....
    qsort( ( void * ) taus, n, sizeof( double ), 
        ( int ( * )( const void *, const void * ) ) compare );
    
    // remove duplicate time points in a complicated way..
    i = 0;
    while( i < M ) {
        // if we're not at the near-end
        if( i < M - 1 ) {
            // is delay i and i+1 close enough to each other? Or, the delay i
            // is bigger than delta time 
            if( ( fabs( delay_array[ i ] - delay_array[ i+1 ] ) < 1E-10 )
                || ( delay_array[ i ] > range ) ) {
                // move all remaining elements one to the back
                memmove( ( delay_array + i ), ( delay_array + i + 1 ), 
                    ( M - i - 1 ) * sizeof( double ) );
                /* printf( "DBG Shifted %d elements to %d\n", M-i-1, i ); */
                M--;
                i--;
            }
        ...
        i++;
        // whatever happens, we do a re-allocation
        delay_array = ( double * ) realloc( delay_array, M * sizeof( double ) );
    }
    /* for( i = 0; i < M; i++ ) 
        printf( "DBG Filtered: %1.14f\n", delay_array[ i ] );
    */

    // set the size of the discontinuities array, and return delay_array
    *disc_size = M;
    return delay_array;
}

// what does this function do? continuous extension...
void
CE( double t, double *vans, double tbegin, double *v_at_tbegin, 
    double ech, double *d1, double *d2, double *d3, double *d4, int n ) {
    
    static double e1 = -4.0 / 3.0;
    static double e2 = 5.0 / 9.0; 
    static double e3 = -2.0 / 3.0;
    static double e4 = -8.0 / 9.0;
...
y_delayed( double ***vd, int n, double *rktimes, double *tau,
    double *grid, double **vdone, double **deriv1, double **deriv2, 
    double **deriv3, double **deriv4, int gridsize, double accu, 
    SolverInput *si, Input *inp ) {
    ...
    static double b32 = 0.75;
    static double c1 = 2.0 / 9.0;
    static double c2 = 1.0 / 3.0;
    static double c3 = 4.0 / 9.0;
 ...
 *
 * vatt     - array for storing results, vatt[0] contains initial conditions
 * n        - size
 * tarray   - array with time points
 * tpoints  - size of time points array
 * darray   - discontinuities array
 * dpoints  - size of discon array
 * stephint - starting step size
 * accuracy - solver accuracy
 * si       - solver input stuff (structure)
 * inp      - input structure
 */
void
DCERk32( double **vatt, int n, double *tarray, int tpoints, double *darray, 
    int dpoints, double stephint, double accuracy, SolverInput *si, 
    Input *inp ) {

    // local loop counters
    int i, dc, vc;
    // where in the t array we are, starting at the value right after the 
    // starting time
    int tpos = 1;               
    // where is the discontinuities array we are (darray)
    int dpos = 0;
    // if we just hit a discontinuity
    int DISCON = 0;             
    // storing intermediate steps
    double **v; 
    // used to toggle between v[0] and v[1]
    int toggle = 0;
    // guessed intermediate v's
    double *vtemp;
    // ptr to v at current time
    double *vnow;
    // ptr to v at current time + stepsize
    double *vnext;
    // ptr to v at t-delay
    double **v_delayed[4];
    ...
    // error estimate
    double *verror;
    // the maximum error in verror
    double verror_max;
    
    // the current time
    double t;
    // array for the rk step times
    double tms[ 4 ];
    // initial stepsize
    double h = stephint;
    // used to calculate next stepsize
    double hnext;

    // safety margin for decrsg stepsize
    const double SAFETY = 0.9;
    // to prevent huge jumps, see num recipes pg. 719
    const double ERRCON = 5.832e-3;
    ...
    // set current time
    t = tarray[ 0 ];
    // set vnow to initial conditions
    vnow = vatt[ 0 ];
    // set vnext at time + stepsize to v[ 0 ] (all zeroes)
    vnext = v[ 0 ];
    ...
    // printf( "DBG %f\n", step_exponent );
    // printf( "DBG disconny:%d %d\n", dpoints, dpos );

    // if we have discons (dpoints > 0) and dpos is still valid
    if( ( dpoints ) && ( dpos < dpoints ) )
        if( ( darray[dpos] > t ) && ( darray[dpos] <= t + h ) ) {
            // printf( "\n DBG Hit discont at %f, %d\n", darray[ dpos ], dpos );
            h = darray[ dpos ] - t;
            dpos++;
            DISCON = 1;
        }

    // min delay < stepsize < 2 * min delay
     ...
     */
    while( y_delayed( v_delayed, n, tms, delay, tdone, vdonne, 
        derivv1, derivv2, derivv3, derivv4, gridpos + 1, accuracy, si, inp ) ) {
    
        printf( "DELAY Rejected Iteration for [%f,%f]!\n", t, t + h );
        h = 0.5 * h;
        tms[0] = t;
        tms[1] = t + a2 * h;
        tms[2] = t + a3 * h;
    ...
    while( t < tarray[ tpoints-1 ] ) {
        /* Take one step and evaluate the error. Repeat until the resulting 
         * error is less than the desired accuracy
         */
        ...
        while( 1 ) {
            ...
            if( !y_delayed( v_delayed, n, tms, delay, tdone, vdonne, 
                derivv1, derivv2, derivv3, derivv4, gridpos + 1, accuracy, 
                si, inp ) ) {
                ...
                for( i = 0; i < n; i++ ) {
                    vtemp[ i ] = vnow[ i ] + 
                        h * ( b21 * derivv1[ gridpos ][ i ] );
                }
                d_deriv( vtemp, v_delayed[ 1 ], t + a2 * h, derivv2[ gridpos ], 
                    n, si, inp );

                for( i = 0; i < n; i++ )
                    vtemp[i] = vnow[i] + h * ( b32 * derivv2[gridpos][i] );
                d_deriv( vtemp, v_delayed[ 2 ], t + a3 * h, derivv3[gridpos], 
                    n, si, inp );

                /* ... then feed them to the Rk32 formula */
                for( i = 0; i < n; i++ )
                    vnext[i] = vnow[i] + h * ( c1 * derivv1[gridpos][i] + 
                        c2 * derivv2[gridpos][i] + c3 * derivv3[gridpos][i] );
                ...
                // for( i = 0; i < 10; i++ ) {
                //     printf( "DBG %d gridpos=%d; derivv1=%lg; derivv2=%lg; 
                // derivv3=%lg; h=%lg\n", i, gridpos, derivv1[gridpos][i], 
                //     derivv2[gridpos][i], derivv3[gridpos][i], h );
                // }
                d_deriv( vnext, v_delayed[ 3 ], t + h, derivv4[ gridpos ], 
                    n, si, inp );

                // calculate the error estimate using the embedded formula
                for( i = 0; i < n; i++ ) {
                    verror[i] = h * ( dc1 * derivv1[gridpos][i] + 
                        dc2 * derivv2[gridpos][i] + dc3 * derivv3[gridpos][i] + 
                        dc4 * derivv4[gridpos][i] );
                }
                // printf( "DBG verror=%lg, h=%lg, dc1=%lg, deriv1=%lg, 
                // dc2=%lg, deriv2=%lg, dc3=%lg, deriv3=%lg, dc4=%lg,  
                // deriv4=%lg\n", verror[0], h, dc1, derivv1[gridpos][0], 
                //    dc2, derivv2[gridpos][0], dc3, derivv3[gridpos][0], 
                //    dc4, derivv4[gridpos][0]);
                
                // find the maximum error
                verror_max = 0.;
                for( i = 0; i < n; i++ ) {
                    if( fabs( vnext[i] ) > 10000. * DBL_EPSILON ) {
                        verror_max = DMAX( fabs( verror[i] / vnext[i] ), 
                            verror_max );
                        // printf( "VERROR THEN i=%d verror=%lg vnext=%lg\n", 
                        //     i, verror[i], vnext[i]);
                    } else {
                        verror_max = DMAX( fabs( 
                            verror[i] / ( 10000. * DBL_EPSILON ) ), verror_max );
                ...
                /* for(i = 0; i < n; i++) 
                       printf( "DBG The deriv1[%d]=%.10f deriv2[%d]=%.10f \
                       deriv3[%d]=%.10f deriv4[%d]=%.10f verror[%d]=%.10f \
                       verror_max=%.10f \n",i, derivv1[gridpos][i],i, 
                       derivv2[ gridpos ][ i ],i,derivv3[gridpos][i], i ,
                       derivv4[ gridpos ][ i ],i, verror[ i ], verror_max );
                 */
                ...
                // printf( "DBG Step size %f suggested!\n", h );
                if( h < DBL_EPSILON ) 
                    error( "SoDe: delay solver stepsize underflow" );

            } else {
                // printf( "DBG Rejected Iteration for [%f,%f]!\n",t,t+h);
                h = 0.5 * h;
                if( ( h > mindel ) && ( h <= 2. * mindel ) ) h = .5 * h;
                if( DISCON ) {
                    dpos--;
                    DISCON = 0;
                }
        ...
        // advance the current time by last stepsize
        // printf( "DBG The stepsize at time %f is %f error is %f\n", 
        ...
        while( ( tarray[tpos] < t + h ) && ( tpos < tpoints ) ) {
            ...
            // what does CE do?
            CE( tarray[ tpos ], vatt[ tpos ], t, vnow, h, 
                derivv1[ gridpos ], derivv2[ gridpos ], 
                derivv3[ gridpos ], derivv4[ gridpos ], n );
            // printf("Vatt: %d %f %f %f %f %f\n", tpos, tarray[ tpos ],
            //     vatt[ tpos ][ 0 ], vnow[ 0 ], t, t+h );
            tpos += 1;
        }
        ...
        if( t - maxdel > tdone[0] ) {
            while( t - maxdel >= tdone[gridstart] ) gridstart++;
            gridstart--;
        }
        memcpy( vdonne[gridpos], vnow, sizeof( *vnow ) * n );
    ...
    free( v[1] );
    for( vc = 0; vc < 4; vc++ ) {
        for( dc = 0; dc < numdel; dc++ ) free( v_delayed[vc][dc] );
        free( v_delayed[vc] );
    }
 ...
 * t1 - time point 1
 * t2 - time point 2
 * zyg - zygote, struct with lots of info
 */
void
DivideHistory( double t1, double t2, Zygote * zyg ) {
    // pointer to auxilary arrays
    double *blug;
    // i is a counter, size stores the nr of nuclei at t2
    int i, size;

    // test if the size at time t2 > size at time t1
    if( ( size = GetNNucs( &( zyg->defs ), zyg->nnucs, t2, 
        &( zyg->times ) ) * zyg->defs.ngenes ) > GetNNucs( 
        &( zyg->defs ), zyg->nnucs, t1, &( zyg->times ) ) * zyg->defs.ngenes ) {
    
        // for each what??
        for( i = 0; i <= gridpos; i++ ) {
            // forward interpolation of vdonne?
            blug = ( double * ) calloc( size, sizeof( double ) );
            Go_Forward( blug, vdonne[i], GetStartLinIndex( t2, &( zyg->defs ), 
                &( zyg->times ) ), GetStartLinIndex( t1, &( zyg->defs ), 
                &( zyg->times ) ), zyg, zyg->defs.ngenes );
            free( vdonne[i] );
            vdonne[i] = blug;

            // forward interpolation of derivative v1
            blug = ( double * ) calloc( size, sizeof( double ) );
            Go_Forward( blug, derivv1[i], GetStartLinIndex( t2, &( zyg->defs ), 
                &( zyg->times ) ), GetStartLinIndex( t1, &( zyg->defs ), 
                &( zyg->times ) ), zyg, zyg->defs.ngenes );
            free( derivv1[i] );
            derivv1[i] = blug;

            // forward interpolation of derivative v2
            blug = ( double * ) calloc( size, sizeof( double ) );
            Go_Forward( blug, derivv2[i], GetStartLinIndex( t2, &( zyg->defs ), 
                &( zyg->times ) ), GetStartLinIndex( t1, &( zyg->defs ), 
                &( zyg->times ) ), zyg, zyg->defs.ngenes );
            free( derivv2[i] );
            derivv2[i] = blug;

            // forward interpolation of derivative v3
            blug = ( double * ) calloc( size, sizeof( double ) );
            Go_Forward( blug, derivv3[i], GetStartLinIndex( t2, &( zyg->defs ), 
                &( zyg->times ) ), GetStartLinIndex( t1, &( zyg->defs ), 
                &( zyg->times ) ), zyg, zyg->defs.ngenes );
            free( derivv3[i] );
            derivv3[i] = blug;

            // forward interpolation of derivative v4
            blug = ( double * ) calloc( size, sizeof( double ) );
            Go_Forward( blug, derivv4[i], GetStartLinIndex( t2, &( zyg->defs ), 
                &( zyg->times ) ), GetStartLinIndex( t1, &( zyg->defs ), 
                &( zyg->times ) ), zyg, zyg->defs.ngenes );
            free( derivv4[i] );
            derivv4[i] = blug;
...
void
Band( double *vin, double *vout, double tin, double tout, double stephint, 
    double accuracy, int n, FILE * slog, SolverInput * sinput, Input * input ) {
    ...
    /* Direct Band solver looks ahead and then gets confused by the change
    ...
    if( i > -1 ) {
        ...
        if( tout < divtimes[i] - divdurations[i] || 
            fabs( tout - divtimes[i] + divdurations[i] ) < 1e-6 ) {
...
my_f_band( realtype t, N_Vector y, N_Vector ydot, void *extra_data ) {  
    ...
    p_deriv( NV_DATA_S( y ), t, NV_DATA_S( ydot ), NV_LENGTH_S( y ), si, inp );
    return 0;
}
